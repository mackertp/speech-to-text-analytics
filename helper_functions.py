...
def get_transcript(audio_file_name, file_type, folder):
	...
	# opens the audiofile and gathers the transcipt result with word confidence
	with open(join(dirname(os.getcwd()+"/"+folder+"/"), audio_file_name), "rb") as audio_file:
		result = json.dumps(speech_to_text.recognize(audio=audio_file, content_type="audio/"+
			file_type, timestamps=False, word_confidence=True), indent=2)
...
def convert_json_to_data(json_file):
	...
	for partial_dict in results:
		...
		# stitch partial content into larger datasets
		partial_confidence_hub.append(partial_confidence)
		full_transcipt += partial_transcript
		# the word confidence is nested... unnesting it
	...
	return {"transcript":full_transcipt, "confidence":confidence, "words":word_confidence_hub, "chunks":chunks}
...
def analyze_personality(text):
	...
	json_data = json.dumps(personality_insights.profile(text, content_type='text/plain', raw_scores=True, 
...
def format_personality(formatted_dict):
	""" taking the entire formmated dictionary from the JSON, deconstruct it into something readable """
	# these are hardcoded because these are always going to be the keys... unless IBM changes it, then 
	# check this by printing out the formmated_dict
	...
	warnings = formatted_dict["warnings"]
	...
	formatted_personality = {"personality": personality_dicts, "needs": needs_dict, "values": values_dict, "con_prefs": con_pref_dict}
